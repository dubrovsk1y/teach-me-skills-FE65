...
import { PostSelectors, setLikePost, setSavedPost, setSelectedPost, setSelectedImage } from "../../redux/reducers/postReducer";
    ...
    return (
        <div className="card" onClick={onCardClick}>
            ...
            <Popup   
                ...
                position={'top left'}                                                                                                   
                onOpen={() => dispatch(setSelectedImage(image ? image : 'https://img5.goodfon.ru/wallpaper/nbig/d/11/abstraktsiia-fon-tsvet-chernyi.jpg'))}          
            ...
            <div className="card__btns">
                <span onClick={(event) => handleButtonClick('like', event)} className={classNames("material-symbols-outlined", "card__like", {["_active"]: likeStatus === 'like'})}> thumb_up </span>
                <span onClick={(event) => handleButtonClick('dislike', event)} className={classNames("material-symbols-outlined", "card__dislike", {["_active"]: likeStatus === 'dislike'})}> thumb_down </span>
                <span onClick={(event) => handleButtonClick('save', event)} className={classNames('material-symbols-outlined', 'card__save', {['_active']: save})}> bookmark </span>
            </div>
        </div>
    )
}
